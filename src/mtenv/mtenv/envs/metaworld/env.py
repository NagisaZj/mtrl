...
def get_list_of_func_to_make_envs(
            ...
            for name, _ in _get_class_items(current_benchmark):
                if name == env_id:
                    ...
                    if env_id_list[0] == env_id_list[1]:
                        ...
                        new_id=env_id
                    env_id_to_task_map[new_id] = task
    ...
    if benchmark_name =='MT1':
        for j in range(len(env_id_list)):
            env_id_list[j] = env_id_list[j][:-3]+'-%d'%j+env_id_list[j][-3:]
    # print(env_id_list)
